...
const sumExams = exams.reduce((acumulador, sum) => acumulador + sum.score, 0);
console.log (sumExams);
/* 
6.2 Dado el mismo array, haz una suma de todos las notas de los examenes de los
alumnos que esten aprobados usando la función .reduce().
 */
const sumExamsApro = exams.reduce((acumulador, exam) => {
    if (exam.score >=5 ) {
        return acumulador + exam.score;
    }
    return acumulador;
},0);
console.log (sumExamsApro);


//6.3 Dado el mismo array, haz la media de las notas de todos los examenes .reduce().

const mediaExams = exams.reduce((acumulador, exam) => acumulador + exam.score, 0) / exams.length;
